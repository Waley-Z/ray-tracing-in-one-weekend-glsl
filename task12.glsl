...
vec3 ray_color(Ray r, int max_depth) {
    ...
    for (int i = 0; i < max_depth; i++) {
        ...
        if (hit_world(r, Interval(0.001, 1.0 / 0.0), rec)) {
            ...
            vec3 background_color = (1.0 - a) * vec3(1.0, 1.0, 0.91) + a * vec3(0.7, 0.95, 0.7);
...
void main() {
    ...
    float waveFrequency = 0.78;
    float sphereRadius = 0.4;

    vec3 colorStart = vec3(0.39, 0.57, 0.45);
    vec3 colorEnd = vec3(0.86, 0.86, 0.64);

    for (int x = 0; x < 10; x++) {
        for (int z = 0; z < 10; z++) {
            float y = sin(float(x) * waveFrequency) * waveAmplitude +
                sin(float(z) * waveFrequency) * waveAmplitude;

            float gradientFactor = (float(x) + float(z)) / (10.0 + 10.0 - 2.0);
            vec3 materialAlbedo = mix(colorStart, colorEnd, gradientFactor);

            vec3 center = vec3(float(x) - 5.0, y, float(z) - 5.0);
            Material mat = Material(materialAlbedo, METAL, 0.0);
            spheres[num_spheres++] = Sphere(center, sphereRadius, mat);
        }
    }
    ...
    cam.vfov = 30.0;
    cam.lookfrom = vec3(5.0, 2.0, 10.0);
